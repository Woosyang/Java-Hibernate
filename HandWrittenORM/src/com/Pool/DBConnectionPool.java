 ...
 * put the Connection Object into List<>, make the Connection object can be reused multiple
 * times.
 * The initialization of the Connection: put multiple connection objects into the List(Connection Pool)
 * 
 * Get the Connection Object from the Connection Pool: if there are available connection
 * objects in the Connection pool, use the last one in the List as the connection object
 * and remove this connection object from the List which means this Connection Object is
 * in use. If there are no more connection object in the List, create a new Connection
 * Object
 * 
 * Close the Connection: not really close the Connection Object but put the Connection 
 * Object in use back to the Connection Pool
 * 
 * @author Woo
 *
 */
